...
async function convertNotionToSiteData(pageId, from, pageRecordMap) {
  ...
  const rawMetadata = normalizePageBlock(block[pageId])
  ...
  for (let i = 0; i < pageIds.length; i++) {
    const id = pageIds[i]
    const value =
      normalizePageBlock(block[id]) || normalizePageBlock(fetchedBlocks[id])
    const properties =
...
function cleanBlock(item) {
  ...
  if (pageBlock) {
    for (const i in pageBlock) {
      ...
      if (pageBlock[i]?.value) {
        delete pageBlock[i].value?.role
...
// 安全提取页面 Block
function normalizePageBlock(rawBlock) {
  if (!rawBlock) return null
  if (rawBlock.value) return rawBlock.value
  const keys = Object.keys(rawBlock)
  if (keys.length > 0 && rawBlock[keys[0]]?.value) {
    return rawBlock[keys[0]].value
  }
  // console.warn('normalizePageBlock failed to find value', rawBlock)
  return rawBlock?.value || rawBlock
}
...
function normalizeCollection(collection) {
  ...
  if (collection[firstKey]?.value) return collection[firstKey].value
