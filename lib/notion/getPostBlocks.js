...
function convertNotionBlocksToPost(id, blockMap, slice) {
  ...
  for (let i = 0; i < blocksToProcess.length; i++) {
    ...
    if (b?.value?.type === 'sync_block' && b?.value?.children) {
      ...
      childBlocks.forEach((childBlock, index) => {
        ...
        clonePageBlock.block[newBlockId] = childBlock?.value
          ? childBlock
          : { value: childBlock }
  ...
  // 4️⃣ 防御性过滤：删除 value 或 id 缺失的 block，防止 react-notion-x 报错
  for (const bId in clonePageBlock.block) {
    const b = clonePageBlock.block[bId]
    if (!b?.value?.id) {
      delete clonePageBlock.block[bId]
    }
  }
